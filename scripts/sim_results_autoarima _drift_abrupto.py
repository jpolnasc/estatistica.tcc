...
def simulate(i):
    ...
    df_nowcast.to_csv(sim_path + "sim_results_drift_abrupto.csv")
...
def main():
    ...
    for i in range(800, 1000):
        try:
            ...
            print(f"Erro: {e}")
            iteracoes_com_erro.append(i)
            continue

    if iteracoes_com_erro:
        df_iteracoes_com_erro = pd.DataFrame(iteracoes_com_erro,
                                             columns=['Iterações com Erro'])
        df_iteracoes_com_erro.to_csv('iteracoes_com_erro_no_drift.csv',
                                     index=False)
